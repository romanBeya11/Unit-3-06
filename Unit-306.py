...
def animate():
	...
	while(counter <= 50):
		...
		if view['imageview1'].x <= -250:
			view['imageview1'].x = 446
			walks = walks + 1
			if walks == 1:
				view['walks_label'].text = "I have been around the block: {} time!".format(walks)
			else:
				view['walks_label'].text = "I have been around the block: {} times!".format(walks)
